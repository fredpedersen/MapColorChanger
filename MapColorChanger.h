...
#ifndef __MapColorChanger_H__
#define __MapColorChanger_H__
...
//	ASErr TwirlGetParms(AILiveEffectEditParamMessage* pb);
 //   ASErr GoTwirl(AIFilterMessage* pb);
	ASErr GoRecolor(AIMenuMessage* pb);
...
//	void ReleaseSelection(AIArtHandle **matches);
	...
	ASErr RecolorMap(MapData *mapObject, AIArtHandle *mesh);

	// creates sphere at every mesh vertex
	ASErr CreateReferenceSpheres(AIArtHandle mesh);

	boolean DoesDataFitMesh(MapData *mapObject, AIArtHandle *mesh);
